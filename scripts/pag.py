        ...
        class PerturbedAttentionScript(scripts.Script):
            ...
            def ui(self, *args, **kwargs):
                with gr.Accordion(open=False, label=self.title()):
                    ...
                    with InputAccordion(
                        False, label="Override for Hires. fix"
                    ) as hr_override:
                        hr_cfg = gr.Slider(
                            minimum=1.0,
                            maximum=30.0,
                    ...
                    with gr.Row():
                        ...
                        block_id = gr.Number(
                            label="U-Net Block Id", value=0, precision=0, minimum=0
                        )
                        block_list = gr.Text(label="U-Net Block List")
                    with gr.Row():
                        sigma_start = gr.Number(
                            minimum=-1.0, label="Sigma Start", value=-1.0
                        )
                        sigma_end = gr.Number(
                            minimum=-1.0, label="Sigma End", value=-1.0
                        )

                    self.infotext_fields = (
                        (
                            enabled,
                            lambda p: gr.Checkbox.update(value="pag_enabled" in p),
                        ),
                        (scale, "pag_scale"),
                        (rescale_pag, "pag_rescale"),
                        (
                            rescale_mode,
                            lambda p: gr.Dropdown.update(
                                value=p.get("pag_rescale_mode", "full")
                            ),
                        ),
                        (adaptive_scale, "pag_adaptive_scale"),
                        (
                            hr_override,
                            lambda p: gr.Checkbox.update(value="pag_hr_override" in p),
                        ),
                        (hr_cfg, "pag_hr_cfg"),
                        ...
                        (
                            hr_rescale_mode,
                            lambda p: gr.Dropdown.update(
                                value=p.get("pag_hr_rescale_mode", "full")
                            ),
                        ),
                        ...
                        (
                            block,
                            lambda p: gr.Dropdown.update(
                                value=p.get("pag_block", "middle")
                            ),
                        ),
                        (block_id, "pag_block_id"),
                        (
                            block_list,
                            lambda p: gr.Text.update(value=p.get("pag_block_list", "")),
                        ),
            ...
            def process_before_every_sampling(self, p, *script_args, **kwargs):
                ...
                if hr_enabled and p.is_hr_pass and hr_override:
                    ...
                    p.cfg_scale = hr_cfg
                    unet = opPerturbedAttention.patch(
                        unet,
                        hr_scale,
                        hr_adaptive_scale,
                        block,
